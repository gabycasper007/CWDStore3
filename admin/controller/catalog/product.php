...
class ControllerCatalogProduct extends Controller {
	private $error = array(); 
  	...
  	public function edit_list() {
		...
		if (isset($this->request->post['selected']) && $this->validateEditList()) {
			...
			foreach ($this->request->post['selected'] as $product_id) {
				...
				$sort_order_str = $product_id.'_sort_order';
				...
				$el_data = array('price' => $price, 'special' => $special, 'quantity' => $quantity, 'model' => $model, 'name' => $name, 'sort_order' => $sort_order, 'status' => $status);
  	...
  	protected function getList() {				
		...
		// End Of admin_catalog_view_links
	    ...
	    $this->data['text_way'] = $this->language->get('text_way');
	    $this->data['text_mod_increase'] = $this->language->get('text_mod_increase');
	    $this->data['text_mod_discount'] = $this->language->get('text_mod_discount');
	    $this->data['button_open_my_content'] = $this->language->get('button_open_my_content');
	    $this->data['button_close_my_content'] = $this->language->get('button_close_my_content');

		$this->data['button_edit_list'] = $this->language->get('button_edit_list');
		$this->data['text_special'] = $this->language->get('text_special');
		$this->data['text_view_in_store'] = $this->language->get('text_view_in_store');
		...
		//$this->data['home_sort_order'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=p.home_sort_order' . $url, 'SSL');
  	...
  	protected function getForm() {
		...
		$this->data['entry_resigilate'] = $this->language->get('entry_resigilate');

		$this->data['entry_title_models'] = $this->language->get('entry_title_models');
		$this->data['entry_title_prices'] = $this->language->get('entry_title_prices');
		$this->data['entry_title_availability'] = $this->language->get('entry_title_availability');
		$this->data['entry_title_dimensions'] = $this->language->get('entry_title_dimensions');
    	...
    	$this->data['entry_alldb'] = $this->language->get('entry_alldb');
		...
		//$this->data['entry_home_sort_order'] = $this->language->get('entry_home_sort_order');
		...
		$this->data['text_change_meta_tags'] = $this->language->get('text_change_meta_tags');
		$this->data['text_change_meta_tags_description'] = $this->language->get('text_change_meta_tags_description');
		$this->data['text_title_document'] = $this->language->get('text_title_document');
		$this->data['text_title_document_description'] = $this->language->get('text_title_document_description');
		$this->data['text_heading_1'] = $this->language->get('text_heading_1');
		$this->data['text_heading_1_description'] = $this->language->get('text_heading_1_description');
		$this->data['text_heading_2'] = $this->language->get('text_heading_2');
		$this->data['text_heading_2_description'] = $this->language->get('text_heading_2_description');
		...
		if (isset($this->error['shortdesc'])) {
			$this->data['error_shortdesc'] = $this->error['shortdesc'];
		} else {
			$this->data['error_shortdesc'] = array();
		}
    	...
    	if (isset($product_info)) $this->data['product_id'] = $product_info['product_id'];
		...
		if (isset($this->request->post['keyword'])) {
			...
			if ($product_info['keyword'])
				...
				$this->data['keyword'] = str_replace(' ', '-', strtolower($this->data['product_description'][2]['name'])) . '-'. $this->request->get['product_id'];
		} else {
			$this->data['keyword'] = '';
		}
		...
		/*if (isset($this->request->post['home_sort_order'])) {
      		$this->data['home_sort_order'] = $this->request->post['home_sort_order'];
    	} elseif (!empty($product_info)) {
      		$this->data['home_sort_order'] = $product_info['home_sort_order'];
    	} else {
			$this->data['home_sort_order'] = 1;
		}*/
			...
			if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
				...
				foreach ($product_option['product_option_value'] as $product_option_value) {
					$product_option_value_data[] = array(
						...
						'o_description'           => $product_option_value['o_description'],
						//'alldb'					  => $product_option_value['alldb'],
  	...
  	public function discountAll() {
  	    ...
  	    foreach($products as $product) {
  	        ...
  	        if($this->request->post['type'] == 1) {
	  	      	...
	  	      	if($this->request->post['way'] == 1) { // increase
	  	        	$price = (float)((float)$product['price'] + (float) $this->request->post['discount']);
	  	        }

	  	        elseif ($this->request->post['way'] == 2) { // discount
	  	        	$price = (float)((float)$product['price'] - (float) $this->request->post['discount']);	
	  	        }
  	        }
			...
			if(($price > 0) && ($price != $product['price'])) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_special(product_id, customer_group_id, price, date_start, date_end) VALUES('" . (int)$product['product_id'] . "', 1, '" . (float)$price . "','".$this->request->post['date_start']."','".$this->request->post['date_end']."')");
				$this->cache->delete('product');
			}

   	      	/* elseif ($this->request->post['way'] == 1) {
