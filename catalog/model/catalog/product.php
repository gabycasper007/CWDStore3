...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			return array(
				...
				'desc_resigilate'        => $query->row['desc_resigilate'],
				...
				'home_sort_order'       => $query->row['home_sort_order'],
	...
	public function getProducts($data = array()) {
		...
		if ($this->config->get('searchanise_status') == 'A' && isset($data['pid'])) {
		...
		if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
			if ($data['sort'] == 'pd.name' || $data['sort'] == 'p.model') {
				...
				$this->config->get('searchanise_status') == 'A' && isset($data['sort']) 
	...
	public function getbestrated($limit) {
		...
		$query = $this->db->query("SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) INNER JOIN " . DB_PREFIX . "review r ON (p.product_id = r.product_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY r.rating DESC LIMIT " . (int)$limit);
	...
	/*public function getPopularProducts($limit) {
		...
		$product_data = $this->cache->get('product.popularproducts.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id'). '.' . $customer_group_id . '.' . (int)$limit);
		if (!$product_data) { 
			$product_data = array();
			...
			$this->cache->set('product.popularproducts.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id'). '.' . $customer_group_id . '.' . (int)$limit, $product_data);
		}
		...
		foreach ($product_option_query->rows as $product_option) {
			if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
				...
				$product_option_data[] = array(
					...
					'option_val'        => $product_option['option_value'],
	...
	public function getAutoProductRelated($product_id, $how_many_roducts) {
		$product_data = array();

		$query = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "product_to_category AS pc WHERE pc.product_id ='" . $product_id . "'");
		$category_id = $query->row['category_id'];

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category AS pc WHERE pc.category_id = '" . $category_id . "' AND pc.product_id !=" . $product_id . "  ORDER BY RAND() LIMIT " . $how_many_roducts);
	...
	public function getProductFeaturedInMenu($category_id) {
		$product_data = array();
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_menu_featured pr LEFT JOIN " . DB_PREFIX . "product p ON (pr.menu_featured_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pr.category_id = '" . (int)$category_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
		
		foreach ($query->rows as $result) { 
			$product_data[$result['menu_featured_id']] = $this->getProduct($result['menu_featured_id']);
		}
	...
	public function getTotalProducts($data = array()) {
		if ($this->config->get('searchanise_status') == 'A' && isset($data['total'])) {
