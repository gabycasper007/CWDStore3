...
class ControllerModuleSearchanise extends Controller {
	...
	public function install() {
		...
		$this->model_setting_setting->editSetting('searchanise', array('searchanise_status' => 'A'));
	}
	...
	public function uninstall() {
		...
		Searchanise::seSendAddonStatusRequest('deleted');
	}
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (isset($this->request->post['searchanise_status'])) {
				$new_status = $this->request->post['searchanise_status'];
				$this->model_setting_setting->editSetting('searchanise', array('searchanise_status' => $new_status));
	...
	private function formationAddonData() {
		$data = array(
			...
			'se_status'          => $this->config->get('searchanise_status'),
			'token'              => $this->session->data['token']
		);

		$stores_count = $this->model_setting_store->getTotalStores();

		if ($stores_count > 0 && Searchanise::seIsRegistered() && $data['se_status'] == 'A') {
