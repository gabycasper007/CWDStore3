  ...
  <div class="box">
    <div class="heading">
      <h1>
        ...
        <?php echo $heading_title . ' &raquo; ';
        foreach ($categories as $category) {
          if ($category['category_id'] == $parent_id) echo $category['name'] . ' &raquo; ';
        }
        foreach ($languages as $language)
          echo isset($category_description[$language['language_id']]) ? $category_description[$language['language_id']]['name'] : ''; 
        ?>
      </h1>
      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        ...
        <div id="tab-data">
          <table class="form">
            ...
            <tr>
              <td><?php echo $entry_menu_featured_product; ?></td>
              <td><input type="text" name="menu_featured" value="" /></td>
            </tr>
            <tr>
              <td>&nbsp;</td>
              <td><div id="product_menu_featured" class="scrollbox">
                  <?php $class = 'odd'; ?>
                  <?php foreach ($product_menu_featured as $product_menu_featured) { 
                            $class = ($class == 'even' ? 'odd' : 'even'); ?>
                            <div id="product_menu_featured<?php echo $product_menu_featured['product_id']; ?>" class="<?php echo $class; ?>"> <?php echo $product_menu_featured['name']; ?><img src="view/image/delete.png" alt="" />
                              <input type="hidden" name="product_menu_featured[]" value="<?php echo $product_menu_featured['product_id']; ?>" />
                            </div>
                  <?php } ?>
                </div></td>
            </tr>
            ...
            <tr>
              ...
              <td valign="top">
                <div class="image">
                  <img src="<?php echo $icon; ?>" alt="" id="icon" />
                  <input type="hidden" name="icon_image" value="<?php echo $icon_image; ?>" id="icon_image" />
                  <br />
                  <a onclick="image_upload('icon_image', 'icon');"><?php echo $text_browse; ?></a>&nbsp;&nbsp;|&nbsp;&nbsp;<a onclick="$('#icon').attr('src', '<?php echo $no_image; ?>'); $('#icon_image').attr('value', '');"><?php echo $text_clear; ?></a>
                </div>
              </td>
            </tr>
            <tr>
              <td><?php echo $entry_promo_image; ?></td>
              <td valign="top">
                <div class="image">
                  <img src="<?php echo $promo; ?>" alt="" id="promo" />
                  <input type="hidden" name="promo_image" value="<?php echo $promo_image; ?>" id="promo_image" />
                  <br />
                  <a onclick="image_upload('promo_image', 'promo');"><?php echo $text_browse; ?></a>&nbsp;&nbsp;|&nbsp;&nbsp;<a onclick="$('#promo').attr('src', '<?php echo $no_image; ?>'); $('#promo_image').attr('value', '');"><?php echo $text_clear; ?></a>
                </div>
              </td>
            </tr>
            <tr>
...
// Related
$('input[name=\'menu_featured\']').autocomplete({
  delay: 500,
  source: function(request, response) {
    $.ajax({
  ...
  select: function(event, ui) {
    $('#product_menu_featured' + ui.item.value).remove();
    
    $('#product_menu_featured').append('<div id="product_menu_featured' + ui.item.value + '">' + ui.item.label + '<img src="view/image/delete.png" alt="" /><input type="hidden" name="product_menu_featured[]" value="' + ui.item.value + '" /></div>');
    $('#product_menu_featured div:odd').attr('class', 'odd');
    $('#product_menu_featured div:even').attr('class', 'even');
...
$('#product_menu_featured div img').live('click', function() {
  $(this).parent().remove();
