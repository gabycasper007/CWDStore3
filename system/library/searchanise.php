...
class Searchanise {
	...
	const SE_PRODUCTS_PER_PASS = 200;
	...
	public static function seClearUnusedEnginesData() {
		$private_keys = array();
		$engines_data = self::seGetEnginesData();
		...
		$query = self::$db->query(
			"SELECT * FROM `" . DB_PREFIX . "se_settings` "
			. " WHERE name = 'private_key' AND value NOT IN ('" . implode("','", $private_keys) . "')");

		$keys_to_delete = $query->rows;
		...
		foreach ($keys_to_delete as $k) {
			self::seSendRequest('/api/state/update', $k['value'], array('addon_status' => 'deleted'));

			self::seDeleteEngineSettings($k['store_id'], $k['lang_code']);
		}
	}
