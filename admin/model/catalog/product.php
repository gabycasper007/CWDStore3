...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . 
			...
			//"', home_sort_order = '" . (int)$data['home_sort_order'] . 
		...
		foreach ($data['product_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . 
				"product_description SET product_id = '" . (int)$product_id . 
				"', language_id = '" . (int)$language_id . 
				"', name = '" . $this->db->escape($value['name']) . 
				"', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . 
				"', meta_description = '" . $this->db->escape($value['meta_description']) . 
				"', description = '" . $this->db->escape($value['description']) . 
				"', desc_resigilate = '" . $this->db->escape($value['desc_resigilate']) . 
				"', shortdesc = '" . $this->db->escape($value['shortdesc']) . 
					...
					if (isset($product_option['product_option_value']) && count($product_option['product_option_value']) > 0 ) {
						foreach ($product_option['product_option_value'] as $product_option_value) {
							$this->db->query("INSERT INTO " . DB_PREFIX . 
								...
								"', o_description = '" . $this->db->escape($product_option_value['o_description']) . 
								//"', alldb = '" . $this->db->escape($product_option_value['alldb']) .  
		...
		if ($data['keyword']) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'product_id=" . (int)$product_id . "', keyword = '" . 
			$this->db->escape( str_replace(' ', '-', strtolower($data['product_description'][2]['name'])) . '-'. $product_id) . "'");
	...
	public function editList($product_id, $el_data) { 
		if (isset($el_data['price']) && isset($el_data['quantity']) && isset($el_data['model']) && isset($el_data['status'])) {	
				...
				"product SET price = '" . (float)$el_data['price'] . 
				"', quantity = '" . (int)$el_data['quantity'] . 

				"', sort_order = '" . (int)$el_data['sort_order'] . 
				
				"', model = '" . $this->db->escape($el_data['model']) . 
				"', status = '" . (int)$el_data['status'] . 
			...
			foreach ($data['product_option'] as $product_option) {
				...
				if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
					...
					if (isset($product_option['product_option_value'])  && count($product_option['product_option_value']) > 0 ) {
						foreach ($product_option['product_option_value'] as $product_option_value) {
							$this->db->query("INSERT INTO " . DB_PREFIX . 
								"product_option_value SET product_option_value_id = '" . (int)$product_option_value['product_option_value_id'] . 
	...
	public function getProducts($data = array()) {
		...
		if (!empty($data['filter_price'])) {
			/* MOD OTTITZ */ $sql .= " AND p.price LIKE '%" . $this->db->escape($data['filter_price']) . "%'";
		}
	...
	public function getProductDescriptions($product_id) {
		...
		foreach ($query->rows as $result) {
			$product_description_data[$result['language_id']] = array(
				...
				'desc_resigilate'   => $result['desc_resigilate'],
	...
	public function getProductmenu_featured($category_id) {
		$product_menu_featured_data = array();
		
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_menu_featured WHERE category_id = '" . (int)$category_id . "'");
		
		foreach ($query->rows as $result) {
			$product_menu_featured_data[] = $result['menu_featured_id'];
		}
		
		return $product_menu_featured_data;
	}


	/*public function getProductmenu_featured($product_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_menu_featured WHERE product_id = '" . (int)$product_id . "'");
