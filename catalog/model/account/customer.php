...
class ModelAccountCustomer extends Model {
	public function addCustomer($data) {
		...
		$customer_group_info = $this->model_account_customer_group->getCustomerGroup($customer_group_id);
      	...
      	$this->db->query("INSERT INTO " . DB_PREFIX . 
      		"customer SET store_id = '" . (int)$this->config->get('config_store_id') . 
		...
		if ($this->config->get('config_account_mail')) {
			...
			if ($data['firma']) {
				$message .= $this->language->get('text_firma') . ' '  . $data['firma'] . "\n";
			}
			if ($data['cif']) {
				$message .= $this->language->get('text_cif') . ' '  . $data['cif'] . "\n";
			}
			if ($data['recom']) {
				$message .= $this->language->get('text_recom') . ' '  . $data['recom'] . "\n";
			}
			if ($data['banca']) {
				$message .= $this->language->get('text_banca') . ' '  . $data['banca'] . "\n";
			}
			if ($data['cont']) {
				$message .= $this->language->get('text_cont') . ' '  . $data['cont'] . "\n";
			}
	...
	public function editPassword($email, $password) {
      	$this->db->query("UPDATE " . DB_PREFIX . 
      		"customer SET salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . 
