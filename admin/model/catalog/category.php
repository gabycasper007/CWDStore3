...
class ModelCatalogCategory extends Model {
	public function addCategory($data) {
		...
		if (isset($data['icon_image'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "category SET icon_image = '" . $this->db->escape(html_entity_decode($data['icon_image'], ENT_QUOTES, 'UTF-8')) . "' WHERE category_id = '" . (int)$category_id . "'");
		}

		if (isset($data['promo_image'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "category SET promo_image = '" . $this->db->escape(html_entity_decode($data['promo_image'], ENT_QUOTES, 'UTF-8')) . "' WHERE category_id = '" . (int)$category_id . "'");
		}
		...
		if (isset($data['product_menu_featured'])) {
			foreach ($data['product_menu_featured'] as $menu_featured_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_menu_featured SET category_id = '" . (int)$category_id . "', menu_featured_id = '" . (int)$menu_featured_id . "'");
			}
		}
	...
	public function editCategory($category_id, $data) {
		...
		// Product Menu Featured
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_menu_featured WHERE category_id = '" . (int)$category_id . "'");
	...
	public function getCategoryMenuFeatured($category_id) {
		$category_menu_featured_data = array();
		
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_menu_featured WHERE category_id = '" . (int)$category_id . "'");
		
		foreach ($query->rows as $result) {
			$category_menu_featured_data[] = $result['menu_featured_id'];
		}
		
		return $category_menu_featured_data;
